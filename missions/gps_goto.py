"""
GPS goto mission: arm → AUTO.TAKEOFF → OFFBOARD → fly to goal → hover → AUTO.LAND.

Goal position is stored in goal.json (same directory as this script).

Usage:
  python3 gps_goto.py                          # fly to saved goal
  python3 gps_goto.py --lat 56.04 --lon 14.15 # set new goal and fly
  python3 gps_goto.py --set-goal               # average current GPS, save as goal, exit
...
This avoids the large bearing errors caused by GPS position noise.
...
GPS_AVG_SAMPLES  = 20   # unique GPS fixes to average (GPS ~5 Hz → ~4 s on the ground)
...
class GpsGoto(Node):
    ...
    def _goto_goal(self):
        ...
        while time.monotonic() < deadline:
            ...
            self.get_logger().info(
                ...
                f'dist={dist:.1f}m  spd={speed:.2f}  ve={ve:.2f}  vn={vn:.2f}',
    ...
    def run(self):
        ...
        # The drone is stationary here, so this is the best time to average:
        # GPS has converged, no vibration, and no movement-induced position drift.
        avg_lat, avg_lon = self._avg_gps_samples(GPS_AVG_SAMPLES)
...
def main():
    parser = argparse.ArgumentParser(description='GPS goto mission')
    ...
    # -- set-goal mode: capture GPS, save, exit --
    if args.set_goal:
        node = GpsGoto(None, None)
        try:
            node.set_goal_mode()
    ...
    node = GpsGoto(goal_lat, goal_lon)
    try:
        node.run()
    except KeyboardInterrupt:
