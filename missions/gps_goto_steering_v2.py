"""
GPS goto mission with continuous yaw steering v2.
...
PPO integration: replace the yaw_rate line in _goto_goal() with:
  yaw_rate = ppo_model_output * YAW_RATE_MAX

Goal position is stored in goal.json (same directory as this script).

Usage:
  python3 gps_goto_steering_v2.py                          # fly to saved goal
  python3 gps_goto_steering_v2.py --lat 56.04 --lon 14.15 # set new goal and fly
  python3 gps_goto_steering_v2.py --set-goal               # average current GPS, save as goal, exit
...
CRUISE_SPEED  = 1.0    # m/s — forward speed
...
class GpsGotoSteeringV2(Node):
    def __init__(self, goal_lat, goal_lon):
        super().__init__('gps_goto_steering_v2')
    ...
    def _goto_goal(self):
        ...
        PPO integration: replace the yaw_rate computation with:
            yaw_rate = ppo_model_output * YAW_RATE_MAX
        """
        dt       = 1.0 / SETPOINT_HZ
        ...
        while time.monotonic() < deadline:
            ...
            # ↑ PPO integration point: replace this line with ppo_model_output * YAW_RATE_MAX
...
def main():
    parser = argparse.ArgumentParser(description='GPS goto mission v2 — continuous yaw steering')
    ...
    if args.set_goal:
        node = GpsGotoSteeringV2(None, None)
        try:
            node.set_goal_mode()
    ...
    print('=' * 56)
    print('GPS goto v2 — continuous yaw steering')
    print()
    ...
    node = GpsGotoSteeringV2(goal_lat, goal_lon)
    try:
        node.run()
    except KeyboardInterrupt:
